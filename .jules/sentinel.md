...
## 2026-07-24 - Prevent DOM XSS by removing innerHTML\n**Vulnerability:** Use of innerHTML for search toggles\n**Learning:** The #createSearchToggle method used innerHTML to set the content of buttons. While it was currently being passed static HTML, using innerHTML is an anti-pattern that creates a DOM XSS sink if untrusted data is ever passed.\n**Prevention:** Create DOM nodes explicitly using document.createElement/createTextNode and append them instead of taking string representations.

## $(date +%Y-%m-%d) - Prevent SVG Mutation XSS in sanitizeString
**Vulnerability:** The `sanitizeString` utility stripped many dangerous tags (like `<script>`, `<object>`, `<iframe>`), but missed `<set>` and `<animate>`, allowing them to manipulate attributes (like `onmouseover` or `href`) when embedded in inline SVGs. Furthermore, `data:application/xml` was not blocked, allowing malicious XML/SVG documents to execute XSS scripts when clicked.
**Learning:** HTML sanitizer logic can easily fall behind newly recognized bypass techniques in SVG. It's critical to consider SVG-specific tags that modify the DOM rather than just traditional HTML-based script execution vectors.
**Prevention:** Always maintain a comprehensive blocklist of active SVG animation tags (`set`, `animate`) and block all active XML-like data URIs when allowing raw strings to be embedded within HTML. Ensure regular review of new bypass methodologies.
