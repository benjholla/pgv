...
## 2024-09-13 - [A* Open List Optimization]
**Learning:** In highly dense graphs with many obstacles, the orthogonal edge routing algorithm's A* open list grows significantly. The previous implementation used an O(N) linear array scan combined with a swap-pop to find the node with the lowest `f` cost. While extremely fast for small arrays due to memory locality and lack of allocation overhead, this creates a significant performance bottleneck (O(N^2) overall expansion) as the graph size increases.
**Action:** Replaced the array scan with a custom, module-level `RouteMinHeap` (O(log N) extraction). Crucially, the heap class is defined at the module scope rather than inside the `routeEdgeOrthogonal` function to prevent V8 hidden class churn and unnecessary object allocation per routing call. Measurement showed a ~40% performance improvement on complex/blocked routing paths without negatively impacting simpler paths.
