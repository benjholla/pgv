...
## 2024-05-18 - Replacing O(N) array scan with O(log N) MinHeap for A* pathfinding
**Learning:** In `@pgv/graph-core`, the `routeEdgeOrthogonal` A* pathfinding algorithm in `src/layout.ts` was using a standard array for its `openList` and performing an O(N) linear scan to find the node with the minimum `f` score. While slightly faster than O(N log N) sorting on very small sets, this linear scan creates a severe bottleneck on dense graph routing where the open list grows significantly. Replacing this with a custom `MinHeap` ensures O(log N) extraction and significantly speeds up pathfinding.
**Action:** Always prefer a priority queue (like a MinHeap) for the `openList` in A* pathfinding to guarantee O(log N) minimum extraction, avoiding O(N) linear array scans that bottleneck dense graph routing performance.
