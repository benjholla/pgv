...
## 2024-11-20 - Custom MinHeap for Priority Queue in A* Pathfinding
**Learning:** The `routeEdgeOrthogonal` A* pathfinding algorithm in `src/layout.ts` was using a linear array scan with a swap-pop to extract the minimum `f` score node from the `openList`. For small grids, this O(N) scan was fast due to cache locality and the low constant factor. However, for large, dense graphs with many nodes/obstacles, the `openList` can grow significantly, making the O(N) minimum extraction a bottleneck.
**Action:** Replaced the array scan with a custom, module-level `RouteMinHeap` (to avoid class allocations per function call) which guarantees O(log N) minimum extraction. Benchmarks verified a substantial reduction in iterations and consistent speedups in deep pathfinding scenarios.
