...
## 2025-02-12 - A* Priority Queue Optimization
**Learning:** In `@pgv/graph-core`, the `routeEdgeOrthogonal` A* pathfinding algorithm originally used a linear `O(N)` scan (`for` loop over `openList`) combined with a swap-and-pop technique to find the node with the lowest `f` score. On densely connected graphs or complex layouts where the `openList` grows large, this `O(N)` scan severely bottlenecks routing performance.
**Action:** Replace the `O(N)` scan array logic in A* `openList` with a standard `MinHeap` data structure. Extracting the minimum `f` score becomes `O(log N)`, drastically speeding up pathfinding and overall graph layout times on complex topologies without sacrificing path correctness.
