...
## 2024-05-19 - Replace O(N) array scan with O(log N) MinHeap for A* openList
**Learning:** In `@pgv/graph-core`, `routeEdgeOrthogonal` calculates shortest paths using A*. The `openList` was previously an array, and finding the node with the minimum `f` score required an `O(N)` linear scan (with a swap-pop to remove it). This becomes a severe bottleneck in dense graphs with many nodes/edges because the open list grows significantly, slowing down the routing algorithm substantially.
**Action:** Replaced the plain array with a custom `MinHeap` implementation for the `openList` to ensure minimum `f` score extraction occurs in `O(log N)` time. Benchmarks show a significant ~30x speedup in the `routeEdgeOrthogonal` performance (bench-astar-full.bench.ts) on typical layouts. Always use priority queues (like a MinHeap) for A* open lists to prevent `O(N)` scans in hot paths.
