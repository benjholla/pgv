...
>> Implemented a custom MinHeap class at the module level in src/layout.ts to optimize the A* pathfinding algorithm for orthogonal edge routing. Replaced the O(N) linear array scan with an O(log N) MinHeap, which significantly improves algorithmic efficiency and scalability, especially for dense graphs where the openList grows large.
