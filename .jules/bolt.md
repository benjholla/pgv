...
## 2025-02-17 - A* OpenList extraction MinHeap vs Array scan
**Learning:** In `@pgv/graph-core`, the `routeEdgeOrthogonal` A* pathfinding algorithm relied on a linear O(N) array scan + swap-pop to find the node with the minimum `f` score in its `openList`. While this is faster than full O(N log N) sorting for small arrays, it becomes a severe bottleneck in dense graphs where the `openList` grows large.
**Action:** Replace the O(N) scan inside A* pathfinding with a custom `MinHeap` priority queue to guarantee O(log N) extraction times. Benchmarks show this makes extraction up to ~7.9x faster in dense graphs, significantly improving layout rendering times for complex topologies.
