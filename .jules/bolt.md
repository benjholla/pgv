...
## 2025-02-13 - Ensure AbortControllers are cleared in renderer teardown
**Learning:** In `@pgv/graph-core`, components interacting with DOM events like dropdowns and resize observers use `AbortController` for managing event listener lifecycles. Missing an `abort()` call and failing to clear the reference during teardown (e.g., in `GraphView.destroy()`) leaves dangling listeners on the DOM, resulting in memory leaks and unexpected behavior when views are re-initialized.
**Action:** Ensure every `AbortController` instantiated within a class has a corresponding `abort()` call and is set to `null` in the class's `destroy` or teardown method to ensure resources are properly released.
