...
## 2026-08-22 - Replaced O(N) array scan with MinHeap in A* routing
**Learning:** The A* edge routing algorithm in `src/layout.ts` previously used a simple array for its open list and an O(N) linear scan to find the node with the minimum 'f' score. This caused extreme performance degradation (14+ seconds) on dense graphs with large open lists. Replacing it with a standard MinHeap reduced the complexity to O(log N), dropping routing time to under 0.4s.
**Action:** Use priority queues (like MinHeaps) for A* or Dijkstra's open lists instead of raw arrays whenever graph topologies could become large or complex.
