...
## 2024-12-09 - [Added role="group" and aria-label to interactive control button groups]
**Learning:** In `@pgv/graph-core`, when grouping multiple interactive control buttons in the DOM (e.g., zoom, pan, or miscellaneous groups in `src/renderer.ts`), explicitly adding `role="group"` and an appropriate `aria-label` to the container element provides structural grouping and context for screen readers. This ensures users understand the relationship between the buttons within those groups.
**Action:** When adding new button groups or containers that cluster related controls, ensure they are assigned `role="group"` and an `aria-label` describing the group's purpose.
