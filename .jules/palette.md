...
## 2026-08-20 - [Added role="group" and aria-label to control groups]
**Learning:** In `@pgv/graph-core`, when grouping multiple interactive control buttons in the DOM (e.g., zoom, pan, or miscellaneous groups in `src/renderer.ts`), explicitly adding `role="group"` and an appropriate `aria-label` to the container element provides critical structural grouping and context for screen readers. Without this, users may encounter a disjointed sequence of buttons without understanding they share a related function.
**Action:** Always add `role="group"` and `aria-label` attributes to container elements representing logical sets of UI controls to ensure a more cohesive and accessible experience.
