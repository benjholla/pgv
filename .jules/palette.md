...
## 2026-09-08 - [Added structural grouping to control buttons]
**Learning:** In `@pgv/graph-core`, when grouping multiple interactive control buttons in the DOM (e.g., zoom, pan, or miscellaneous groups in `src/renderer.ts`), it's crucial to explicitly add `role="group"` and an appropriate `aria-label` to the container element. This provides structural grouping and necessary context for screen readers.
**Action:** When creating new control groups, always ensure `role="group"` and `aria-label` are assigned to the container.
