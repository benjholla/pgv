...
## 2026-08-24 - [Add role="group" to button containers]
**Learning:** In `@pgv/graph-core`, when grouping multiple interactive control buttons in the DOM (e.g., zoom, pan, or miscellaneous groups in `src/renderer.ts`), explicitly adding `role="group"` and an appropriate `aria-label` to the container element provides structural grouping and context for screen readers. This improves the clarity of keyboard navigation.
**Action:** When creating container elements for related buttons, apply `role="group"` and an `aria-label` attribute.
