...
describe("Horizontal Routing Boundary", () => {
  ...
  it("Horizontal Alignment Non-Overlap Property: Paths to horizontally aligned children do not perfectly overlap", () => {
    ...
    const baseSourcePt = { x: 150, y: -100 };
    const spacing = 16;

    // As per memory: "when explicitly testing routeEdgeOrthogonal for staggered overlapping behavior, the input sourcePt must be manually staggered to simulate the real layout engine's edgeEndpoints logic"
    const sourcePt1 = { x: baseSourcePt.x - spacing, y: baseSourcePt.y };
    const sourcePt2 = { x: baseSourcePt.x, y: baseSourcePt.y };
    const sourcePt3 = { x: baseSourcePt.x + spacing, y: baseSourcePt.y };
