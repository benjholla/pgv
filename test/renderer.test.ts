...
describe('GraphView', () => {
  ...
  it('initializes and renders a graph with basic layout properties', () => {
    ...
    const view = renderGraph(container, { nodes: [], edges: [] }, snapshot, { layout });
  ...
  it('handles viewport interaction (pan)', () => {
    ...
    const snapshot = createGraphSnapshot(json);
    const layout = verticalLayout(snapshot);
    const view = renderGraph(container, { nodes: [], edges: [] }, snapshot, { layout, usePanZoom: true });
  ...
  it('handles minimap toggle and interactions', async () => {
    ...
    const snapshot = createGraphSnapshot(json);
    const layout = verticalLayout(snapshot);
    const view = renderGraph(container, { nodes: [], edges: [] }, snapshot, { layout, usePanZoom: true, useThemeToggle: true });
  ...
  it('handles theme toggling', async () => {
    ...
    const snapshot = createGraphSnapshot(json);
    const layout = verticalLayout(snapshot);
    const view = renderGraph(container, { nodes: [], edges: [] }, snapshot, { layout, useThemeToggle: true, usePanZoom: true });
  ...
  it('handles graph diff history navigation', async () => {
    ...
    const view = renderGraph(container, { nodes: [], edges: [] }, snapshot, { layout, maxHistory: 10 });
