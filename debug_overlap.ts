...
function segmentsOverlap(pathA: any, pathB: any): boolean {
    ...
    for (let j = 1; j < pathB.length; j++) {
      ...
      if (a1.x === a2.x && b1.x === b2.x && a1.x === b1.x) {
        ...
        if (Math.max(aMin, bMin) < Math.min(aMax, bMax)) {
          console.log(`VOverlap! aX=${a1.x} aY=${aMin}..${aMax} bY=${bMin}..${bMax}`);
          return true;
        }
      }
      ...
      if (a1.y === a2.y && b1.y === b2.y && a1.y === b1.y) {
        ...
        if (Math.max(aMin, bMin) < Math.min(aMax, bMax)) {
          console.log(`HOverlap! aY=${a1.y} aX=${aMin}..${aMax} bX=${bMin}..${bMax}`);
          return true;
        }
      }
    }
...
segmentsOverlap(path1, path2);
segmentsOverlap(path2, path3);
segmentsOverlap(path1, path3);
